...
1. **`pixscale-gateway` (Node.js / NestJS)**: O serviço de borda. Responsável pela autenticação dos clientes (JWT), sanitização de inputs e controle de taxa (*Rate Limiting*). Ele orquestra as validações estruturais consultando o serviço de limites e encaminha as transações de forma assíncrona usando o padrão **Fire & Forget**.
...
3. **`pixscale-worker` (Node.js / NestJS)**: O motor transacional de alto desempenho. Consome os fluxos de eventos do Apache Kafka e garante a **Idempotência** do sistema utilizando o Redis para descartar payloads duplicados instantaneamente.
